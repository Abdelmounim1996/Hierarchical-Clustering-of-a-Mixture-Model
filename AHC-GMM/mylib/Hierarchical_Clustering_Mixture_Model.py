...
class Hierarchical_Mixture_Model :
    ...
    def __init__(self , k :  int ,m : int , n_iters :  int = 10 , tol : float = 0.000001, seed =  0   , mean_F = None, cov_F= None, weight_F= None ):
        self.k         = k                              
        self.m         = m                               
    ...
    def update_PI(self ):
        PI = np.array([list(map(lambda  gauss_G :  self.kl_mvn(gauss_F , gauss_G ) , zip(self.mean_G , self.cov_G) ))
                       for gauss_F in  zip( self.mean_F ,self.cov_F)  ] ).argmin(axis =1 )
    ...
    def update_G(self) :
        ...
        for i , index in PI_inv.items():
            ...
            cov_G[i] = 1/weight_G[i]*np.sum(self.cov_F[index]*self.weight_F[index ][:, np.newaxis , np.newaxis]
                            +list(map(lambda row : row[:, np.newaxis] *row , 
    ...
    def dis_KL_GMM(self):
        return sum(list(map(lambda gauss :  gauss[-1]*self.kl_mvn(gauss[0:2], gauss[2:4]) ,
                zip(self.mean_F, self.cov_F ,self.mean_G[self.PI] ,self.cov_G[self.PI] , self.weight_F  ))))
